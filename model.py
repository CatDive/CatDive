...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, cate_num, category, args):
        ...
        if args.multi:  # initialize multi-embedding
            self.item_emb = torch.nn.Embedding(self.item_num+2, int(args.hidden_units*(1/2)), padding_idx=0)
            self.cate_emb = torch.nn.Embedding(self.cate_num+2, int(args.hidden_units*(1/2)), padding_idx=0)
        else:
    ...
    def log2feats(self, log_seqs):
        ...
        if self.cate_emb: # loads multi-embedding
            ...
            seqs *= (self.item_emb.embedding_dim + self.cate_emb.embedding_dim) ** 0.5
        else:
